...
class ART(nn.Module):
    ...
    def test_forward(self, x):
        output = 0
        org_lq = x.clone()
        h_lq = torch.flip(org_lq, [3])
        v_lq = torch.flip(org_lq, [2])
        hv_lq = torch.flip(org_lq, [2, 3])

        output += self.sub_test(org_lq).detach()

        out = self.sub_test(h_lq)
        output += torch.flip(out.detach(), [3])

        out = self.sub_test(v_lq)
        output += torch.flip(out.detach(), [2])

        out = self.sub_test(hv_lq)
        output += torch.flip(out.detach(), [2, 3])
    ...
    def sub_test(self, x):
        ...
        with torch.no_grad():
            ...
            for chop in img_chops:
                out = self.forward(chop)  # image processing of each partition
                outputs.append(out)
            _img = torch.zeros(1, C, H * scale, W * scale, device=x.device)
            # merge
            for i in range(ral):
                for j in range(row):
